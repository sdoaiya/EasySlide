...
class ServiceConfig:
    ...
    def from_defaults(
        ...
        **kwargs
        ...
        默认配置（推荐用于导出PPTX）：
        - 元素提取：内置 Paddle / MinerU 解析路径，不再默认调用百度 OCR
        ...
        # 导出可编辑 PPTX 需要 MinerU layout.json / bbox，不能用只返回 Markdown 的 Paddle PDF 解析。
        if not mineru_token:
