...
test.describe('Editable export failure UI', () => {
  ...
  test('shows codex relogin toast for oauth 401 failures and keeps it visible for 5s', async ({ page }) => {
    ...
    const reloginToast = page.getByText('Codex 授权已过期或未连接，请前往设置重新连接 OpenAI 授权后再试。');
  ...
  test('lets failed editable exports retry without showing exported files list', async ({ page }) => {
    const projectId = 'mock-editable-export-retry';
    ...
    await page.route(url => new URL(url).pathname.startsWith('/api/'), async route => {
      ...
      if (url.pathname === `/api/projects/${projectId}/export/editable-pptx`) {
        ...
        return route.fulfill({
          ...
          body: JSON.stringify({
            success: true,
            data: { task_id: `editable-export-task-${exportRequests}` },
          }),
        });
      }
      ...
      if (url.pathname === `/api/projects/${projectId}/tasks/editable-export-task-2`) {
        return route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({
            success: true,
            data: {
              task_id: 'editable-export-task-2',
              task_type: 'EXPORT_EDITABLE_PPTX',
              status: 'RUNNING',
              progress: { total: 100, completed: 10, percent: 10 },
            },
          }),
        });
      }
      ...
      if (url.pathname === `/api/projects/${projectId}/exports`) {
        return route.fulfill({
          ...
          body: JSON.stringify({
            ...
            data: {
              files: [{
                filename: 'duplicate.pptx',
                type: 'pptx',
                size: 1234,
                modified_at: new Date().toISOString(),
                download_url: '/files/mock/duplicate.pptx',
              }],
            },
          }),
        });
    ...
    await expect(page.getByText('已导出文件')).toHaveCount(0);
    await page.getByRole('button', { name: '重试' }).click();

    await expect
      .poll(() => exportRequests, { timeout: 10000 })
      .toBe(2);
  });
});
