---
title: "CLI Tool"
description: "Use easyslide-cli to operate EasySlide from the command line"
---

## Overview

`easyslide-cli` is the command-line tool for EasySlide, implementing all features through the backend REST API. Ideal for:
...
uv run easyslide-cli --help  # Verify installation
```

## Agent Skill

If you use AI Agents in Claude Code, install the easyslide-cli skill with a single command so the Agent automatically knows how to use the CLI:

```bash
# Global install (recommended, available in all projects)
npx skills add EasySlide -g

# Or install to current project only
npx skills add EasySlide
```
...
```bash
easyslide-cli [global-options] <command-group> <subcommand> [options]
```
...
| `--config` | Config file path (TOML) | `~/.config/easyslide/cli.toml` |
| `--json` | Output in JSON format | Off |
| `--verbose` | Verbose output | Off |

### Configuration Priority

CLI args > Environment variables (`EASYSLIDE_CLI_*`) > TOML config file > Defaults

Environment variable mapping:

```bash
EASYSLIDE_CLI_BASE_URL=http://localhost:5011
EASYSLIDE_CLI_ACCESS_CODE=your-code
EASYSLIDE_CLI_POLL_INTERVAL=3
EASYSLIDE_CLI_REQUEST_TIMEOUT=60
EASYSLIDE_CLI_CONTINUE_ON_ERROR=true
```
...
```bash
easyslide-cli projects get a1b2          # matches a1b2c3d4-e5f6-...
easyslide-cli pages edit-image --page-id b9c8 --instruction "Change the title to red"
```
...
```bash
easyslide-cli projects use a1b2     # set working project (supports short prefix)
easyslide-cli workflows outline      # uses working project automatically
easyslide-cli projects use           # show current working project
easyslide-cli projects unuse         # clear
```

The working project is stored in `~/.config/easyslide/context.json` and persists across sessions.
...
```bash
# List projects
easyslide-cli projects list --limit 50 --offset 0

# Get project details
easyslide-cli projects get <project_id>

# Create project
easyslide-cli projects create --creation-type idea --idea-prompt "AI technology trends"

# Update project
easyslide-cli projects update <project_id> --template-style "Dark business style"

# Delete project
easyslide-cli projects delete <project_id>

# Set/show working project
easyslide-cli projects use <project_id>
easyslide-cli projects use

# Clear working project
easyslide-cli projects unuse
```
...
```bash
# Generate outline
easyslide-cli workflows outline --project-id <id>

# Specify page count (hint to AI, actual count may differ)
easyslide-cli workflows outline --project-id <id> --pages 8

# Generate outline from descriptions
easyslide-cli workflows outline --project-id <id> --from-description

# Refine outline with natural language
easyslide-cli workflows outline --project-id <id> --refine "Add a page about market analysis"

# Generate descriptions (waits for completion by default; add --no-wait for async)
easyslide-cli workflows descriptions --project-id <id>

# Generate images
easyslide-cli workflows images --project-id <id> --use-template

# Full pipeline: outline -> descriptions -> images
easyslide-cli workflows full --project-id <id> --language zh

# Full pipeline with page count (page count is a hint to AI, actual count may differ)
easyslide-cli workflows full --project-id <id> --language zh --pages 10
```

`workflows full` supports skipping steps:

```bash
easyslide-cli workflows full --project-id <id> \
  --skip-outline \
  --desc-max-workers 4 \
  --image-max-workers 2
```
...
```bash
# Create page
easyslide-cli pages create --project-id <id> --order-index 0

# Set outline content
easyslide-cli pages set-outline --project-id <id> --page-id <pid> \
  --outline-json '{"title": "Chapter 1"}'

# Generate single page description
easyslide-cli pages gen-description --project-id <id> --page-id <pid>

# Generate single page image
easyslide-cli pages gen-image --project-id <id> --page-id <pid> --use-template

# Edit image
easyslide-cli pages edit-image --project-id <id> --page-id <pid> \
  --instruction "Change the title to red"

# View image versions
easyslide-cli pages versions --project-id <id> --page-id <pid>

# Switch image version
easyslide-cli pages set-current --project-id <id> --page-id <pid> --version-id <vid>
```
...
easyslide-cli exports pptx --project-id <id>

# Export and download locally
easyslide-cli exports pptx --project-id <id> --output ./slides.pptx

# Export PDF
easyslide-cli exports pdf --project-id <id> --output ./slides.pdf

# Export images
easyslide-cli exports images --project-id <id>

# Export editable PPTX (waits for completion by default)
easyslide-cli exports editable-pptx --project-id <id> --output ./editable.pptx --max-depth 2
```
...
easyslide-cli materials upload --file /path/to/image.png --project-id <id>

# Upload global material
easyslide-cli materials upload --file /path/to/image.png --global

# AI-generate material
easyslide-cli materials generate --prompt "Tech-style background" --project-id <id> --wait

# Associate global material with project
easyslide-cli materials associate --project-id <id> --material-url <url>

# Download material
easyslide-cli materials download --material-id <mid> --output /tmp/materials.zip
```
...
easyslide-cli refs upload --file /path/to/document.pdf --project-id <id>

# Trigger parsing
easyslide-cli refs parse --file-id <fid>

# Associate with project
easyslide-cli refs associate --file-id <fid> --project-id <id>
```
...
easyslide-cli templates upload --project-id <id> --file /path/to/template.png

# Delete template
easyslide-cli templates delete --project-id <id>
```
...
easyslide-cli renovation create --file /path/to/old.pptx --wait --language zh
```
...
easyslide-cli settings get

# Update settings
easyslide-cli settings update --data '{"TEXT_MODEL": "gemini-2.0-flash"}'

# Reset settings
easyslide-cli settings reset

# Verify configuration
easyslide-cli settings verify

# Run feature test
easyslide-cli settings test --name text-model
```
...
```bash
# Query task status
easyslide-cli tasks status --project-id <id> --task-id <tid>

# Wait for task completion (shows real-time progress)
easyslide-cli tasks wait --project-id <id> --task-id <tid> --timeout-sec 600
```
...
```bash
easyslide-cli styles extract --image /path/to/reference.png
```

### files -- File Download

```bash
easyslide-cli files fetch --url /files/project-id/exports/slides.pptx --output ./slides.pptx
```
...
easyslide-cli run monitor --state-file state.json

# Continuous monitoring until completion
easyslide-cli run monitor --state-file state.json --watch --interval 30
```
...
result=$(easyslide-cli --json projects create --creation-type idea --idea-prompt "2024 AI Development Report")
...
# 3. Export locally
easyslide-cli exports pptx --output ./report.pptx
```
...
# Execute
easyslide-cli run jobs --file batch.jsonl --report report.json --state-file state.json
```
...
7. **Shell completion**: Run `easyslide-cli --install-completion` to install auto-completion for your current shell
