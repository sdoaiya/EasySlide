...
from ..lazyllm_env import DEFAULT_LAZYLLM_NAMESPACE, ensure_lazyllm_namespace_key
...
class LazyLLMTextProvider(TextProvider):
    ...
    def __init__(self, source: str = 'deepseek', model: str = "deepseek-v3-1-terminus"):
        ...
        ensure_lazyllm_namespace_key(source)
        ...
        self.client = lazyllm.namespace(DEFAULT_LAZYLLM_NAMESPACE).OnlineModule(
            source=source,
            model=model,
        )
    ...
    def generate_with_image(self, prompt: str, image_path: str, thinking_budget: int = 0) -> str:
        ...
        if self._vlm_client is None:
            with self._vlm_lock:
                if self._vlm_client is None:
                    import lazyllm
                    ensure_lazyllm_namespace_key(self._source)
                    self._vlm_client = lazyllm.namespace(DEFAULT_LAZYLLM_NAMESPACE).OnlineModule(
