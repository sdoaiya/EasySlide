"""OpenAI Codex OAuth Controller — PKCE authorization flow for OpenAI access.
...
def manual_callback():
    ...
    if error_param:
        return error_response("OPENAI_OAUTH_ERROR", f"OpenAI 授权失败: {error_param}", 400)
    ...
    result = _exchange_and_store(
        ...
        missing_message="授权会话已过期或已使用，请重新连接后粘贴新的 URL",
    )
