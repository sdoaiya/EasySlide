...
export function captureWholeTextElement(el, slideRect, style, slideIndex, clipRect = null) {
  ...
  if (shouldSkipEditableExportElement(el)) return null;
  ...
  if (value.length < 18 || isExportNoiseText(value)) return null;
...
export function captureTextNode(textNode, parent, slideRect, style, slideIndex, clipRect = null) {
  ...
  if (!value.trim() || isExportNoiseText(value) || shouldSkipEditableExportElement(parent)) return null;
  const range = document.createRange();
...
export function visibleTextInSubtree(root, slideRect) {
  const texts = [];
  const walk = (node) => {
    if (node.nodeType === Node.ELEMENT_NODE && shouldSkipEditableExportElement(node)) return;
    for (const child of node.childNodes || []) {
      if (child.nodeType === Node.TEXT_NODE) {
        const text = normalizeText(child.textContent || '');
        if (!text || isExportNoiseText(text)) continue;
        const range = document.createRange();
...
export function visibleTextInScreenshotRect(root, slideRect, target = null) {
  ...
  const add = (text, rect) => {
    ...
    if (!value || isExportNoiseText(value) || rect.width <= 1 || rect.height <= 1) return;
  ...
  const walk = (node) => {
    for (const child of node.childNodes || []) {
      if (child.nodeType === Node.TEXT_NODE) {
        ...
        if (shouldSkipEditableExportElement(child)) continue;
        walk(child);
      }
    }
  };
...
export function isMediaChrome(el) {
  return !!el.closest('script,style,noscript,template,#nav,#preview-panel,#slide-rail,.theme03-theme-toggle,.ctl,.spill,input')
    || shouldSkipEditableExportElement(el);
}

export function shouldSkipEditableExportElement(el) {
  return !!el?.closest?.('[data-editable-skip="true"],[data-editable-skip]');
}
...
export function isExportNoiseText(value) {
