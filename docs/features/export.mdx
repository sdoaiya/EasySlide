...
| Native Editable PPTX | Exports native projects from real HTML into editable PowerPoint objects without MinerU, inpainting, or image-recognition services |
...
Choose **Native editable** when creating a project to store each slide as a controlled layout plus structured content. Text, basic shapes, and images are exported from the real HTML as individual PowerPoint objects. Complex visuals may use local raster fallback, but full-slide raster layers cannot overlap editable text.

Native PPTX generation runs in the browser or desktop app and does not call MinerU, background repair, or image-recognition models. Tasks pause at page boundaries; after an app restart, resuming rebuilds the file from the saved slide structure. The completed task includes a per-page quality report for text, shapes, images, and warnings.
...
3. In the export dialog, configure:
   ...
   - **Narration mode** — choose a single presenter or a two-speaker dialogue between a host and an expert.
   - **Speaker voices** — in dialogue mode, select a separate TTS voice for each role.
...
Narration uses free Microsoft Edge voices through `edge-tts`, with no API key required. Options include Chinese (晓晓, 云希), English (Jenny, Guy), and Japanese (Nanami, Keita).

Dialogue mode generates separate narration segments for the host and expert, synthesizes each segment independently, and combines them with role-aware subtitles. Existing single-speaker narration remains compatible. Repeated exports reuse cached audio when the text, voice, rate, speed, and language are unchanged.
