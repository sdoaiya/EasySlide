...
ENV_MAP = {
    "base_url": ("EASYSLIDE_CLI_BASE_URL", "BANANA_CLI_BASE_URL"),
    "access_code": ("EASYSLIDE_CLI_ACCESS_CODE", "BANANA_CLI_ACCESS_CODE"),
    "poll_interval": ("EASYSLIDE_CLI_POLL_INTERVAL", "BANANA_CLI_POLL_INTERVAL"),
    "request_timeout": ("EASYSLIDE_CLI_REQUEST_TIMEOUT", "BANANA_CLI_REQUEST_TIMEOUT"),
    "continue_on_error": ("EASYSLIDE_CLI_CONTINUE_ON_ERROR", "BANANA_CLI_CONTINUE_ON_ERROR"),
}
...
def default_config_path() -> Path:
    ...
    if appdata:
        root = Path(appdata)
        legacy_path = root / "banana-slides" / "cli.toml"
        if legacy_path.exists():
            return legacy_path
        return root / "easyslide" / "cli.toml"
...
def resolve_config(
    *,
    ...
    for key, env_names in ENV_MAP.items():
        val = next((os.getenv(env_name) for env_name in env_names if os.getenv(env_name) is not None), None)
        if val is None:
            continue
