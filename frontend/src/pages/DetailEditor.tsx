...
export const DetailEditor: React.FC = () => {
  ...
  const [availableFields, setAvailableFields] = useState<string[]>(() => {
    try {
      const stored = localStorage.getItem('easyslide-available-extra-fields');
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const storedLevel = sessionStorage.getItem('easyslide-detail-level');
        ...
        setAvailableFields(prev => {
          ...
          localStorage.setItem('easyslide-available-extra-fields', JSON.stringify(merged));
          return merged;
        });
        // Cache settings in sessionStorage for store to read
        sessionStorage.setItem('easyslide-settings', JSON.stringify(s));
      } catch { /* ignore */ }
    })();
  }, []);
  ...
  const saveSettingsDebounced = useCallback((updates: Record<string, unknown>) => {
    ...
    settingsSaveTimerRef.current = setTimeout(async () => {
      try {
        ...
        if (res.data) {
          sessionStorage.setItem('easyslide-settings', JSON.stringify(res.data));
        }
      } catch (e) {
  ...
  const handleFieldDragEnd = useCallback((event: DragEndEvent) => {
    ...
    localStorage.setItem('easyslide-available-extra-fields', JSON.stringify(nextPool));
    // 激活字段按新池顺序重排
