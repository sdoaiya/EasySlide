...
EasySlide uses Markdown files (`.md` / `.txt`) to import and export outlines and page descriptions. The format is human-readable and easy to edit in any text editor.
...
If the description ends with short extra-field lines such as `Visual Focus: ...` or `排版布局：...`, EasySlide will preserve them on import.
...
Copy the prompt into any AI assistant such as ChatGPT or Claude, replace the placeholders, and ask it to output Markdown only. You can then import that result into EasySlide.

<Tip>
  This is only a helper for creating import files with external AI, not the main EasySlide workflow. In most cases, creating a project directly inside the app and using the built-in AI will be faster.
</Tip>
...
<Tip>
  EasySlide' current built-in prompt is closer to this structure:
...
```text
Generate presentation content in the following Markdown format so it mirrors EasySlide' current built-in prompt style.
