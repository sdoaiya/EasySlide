...
export // 只把 CSS 泛型字体映射到 Office/WPS 的安全字体。真实命名字体需要原样写入
// PPTX,这样 WPS 才能按字体名自动下载或替换成用户安装的同名字体。
const PPTX_SAFE_FONT_MAP = [
  [/^(ui-monospace|monospace|Menlo|Consolas)$/i, 'Courier New'],
  [/^serif$/i, 'Georgia'],
  [/^(sans-serif|system-ui|-apple-system)$/i, 'Arial'],
];
...
export function renderText(slide, node, slideRect, warnings, totals) {
  ...
  if (!value.trim() || isExportNoiseText(value)) return;
  const c = coords(node, slideRect);
  ...
  const outline = textOutlineForStyle(style);
  if (outline) options.outline = outline;
  const textShadow = textShadowForStyle(style);
  if (textShadow) options.shadow = textShadow;
...
export function textOutlineForStyle(style = {}) {
  ...
  return { color: stroke.color, size: round(Math.max(0.25, Math.min(8, strokeWidth * PX_TO_PT))) };
}

export function textShadowForStyle(style = {}) {
  const raw = String(style?.textShadow || '').trim();
