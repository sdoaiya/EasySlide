...
EasySlide 会在运行时依赖中显式打包 LazyLLM 国内在线厂商 SDK：
...
[OpenAI Platform](https://platform.openai.com/api-keys) 是创建 OpenAI 文本和图像模型 API Key 的默认入口。

获取 API Key 时，打开 [API keys 页面](https://platform.openai.com/api-keys)，点击 **Create new secret key** 创建密钥，然后复制到本项目设置页或 `.env`。
...
讲解视频可使用 Fish Audio `s2.1-pro-free` 合成克隆声音。推荐在「设置 → Fish Audio」中保存并测试 Key；也可通过环境变量配置：
...
Key 会加密存储，接口响应、任务恢复参数和音频缓存键均不会包含 Key。模型和官方 API 地址由应用固定，不能在前端改写。

详见 [Fish Audio 旁白与语音克隆](/zh/features/fish-audio)。
