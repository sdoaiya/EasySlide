...
DEFAULT_LAZYLLM_NAMESPACE = "EASYSLIDE"
LEGACY_LAZYLLM_NAMESPACE = "BANANA"


def get_lazyllm_api_key(source: str, namespace: str = DEFAULT_LAZYLLM_NAMESPACE) -> str:
    """
    ...
    if not source_upper:
        return ""
    return (
        os.getenv(f"{source_upper}_API_KEY", "")
        or os.getenv(f"{namespace}_{source_upper}_API_KEY", "")
        or os.getenv(f"{LEGACY_LAZYLLM_NAMESPACE}_{source_upper}_API_KEY", "")
    )


def ensure_lazyllm_namespace_key(source: str, namespace: str = DEFAULT_LAZYLLM_NAMESPACE) -> bool:
    """
