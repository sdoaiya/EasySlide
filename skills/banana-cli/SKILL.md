...
name: easyslide-cli
description: >
  CLI tool for creating, managing, and exporting AI-generated presentations via the EasySlide API.
...
```bash
easyslide-cli <command> [options]
```

If `easyslide-cli` is not on PATH, use `uv run easyslide-cli` from the project root, or install globally with `uv tool install .`

## End-to-End Workflow

```bash
# 1. Create project and set as working project
result=$(easyslide-cli --json projects create --creation-type idea --idea-prompt "Your topic")
project_id=$(echo "$result" | jq -r '.data.project_id')
easyslide-cli projects use "$project_id"

# 2. Generate everything (outline → descriptions → images)
easyslide-cli workflows full --language zh --pages 8

# 3. Export to local file
easyslide-cli exports pptx --output ./slides.pptx
```
...
```bash
easyslide-cli projects get a1b2          # matches a1b2c3d4-...
easyslide-cli pages edit-image --page-id b9c8 --instruction "change title to red"
```
...
```bash
easyslide-cli projects use a1b2     # set (accepts prefix)
easyslide-cli workflows outline      # uses working project
easyslide-cli projects use           # show current
easyslide-cli projects unuse         # clear
```
...
```bash
easyslide-cli workflows outline --pages 5
easyslide-cli workflows full --pages 10 --language en
```
...
easyslide-cli exports pptx --output ./slides.pptx
easyslide-cli exports pdf --output ./report.pdf

# Without --output, returns a server-side download URL
easyslide-cli exports pptx
```
...
easyslide-cli run jobs --file jobs.jsonl --report report.json --state-file state.json
```

### Renovate existing PPT

```bash
easyslide-cli renovation create --file /absolute/path/to/slides.pptx --language zh
```

### JSON output for scripting

```bash
easyslide-cli --json projects list | jq '.data.projects[].project_id'
```
...
- Config priority: CLI args > env vars (`EASYSLIDE_CLI_*`) > TOML config (`~/.config/easyslide/cli.toml`) > defaults

## Discovering Commands

Run `easyslide-cli --help` for the top-level command list, and `easyslide-cli <command> --help` for subcommand options. Help output is plain text when piped (non-TTY).
