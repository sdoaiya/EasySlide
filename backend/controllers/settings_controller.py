...
from services.ai_providers.ocr.paddle_ocr_provider import create_paddle_ocr_provider
...
def _get_test_image_path() -> Path:
    candidates = [
        Path(PROJECT_ROOT) / "assets" / "test_img.png",
        Path(sys.executable).resolve().parent / "assets" / "test_img.png",
        Path(sys.executable).resolve().parent / "_internal" / "assets" / "test_img.png",
    ]
    for test_image in candidates:
...
# 测试函数 - 每个测试一个独立函数
def _test_paddle_ocr():
    """测试 PaddleOCR 服务"""
    provider = create_paddle_ocr_provider()
...
def _test_paddle_pdf():
    """Test built-in Paddle PDF parsing."""
    tmp_file = None
    try:
        ...
        result = create_paddle_ocr_provider().recognize(str(tmp_file), max_wait_time=120)
        return {
            "content_preview": (result.get("markdown_content") or result.get("recognized_text") or "").strip()[:120],
            "page_count": result.get("page_count", 0),
        }, "Paddle PDF parse test succeeded"
    finally:
...
def _is_codex_oauth_unauthorized(error: Exception, test_name: str, test_settings: dict) -> bool:
    ...
    oauth_not_connected = (
        ...
        or "reconnect openai authorization" in error_text
    )
        ...
        try:
            ...
            if task:
                ...
                if _is_codex_oauth_unauthorized(e, test_name, test_settings):
                    ...
                    error_msg = "Codex 授权已过期或无效，已断开 OpenAI 授权连接。请重新连接 OpenAI 授权后再测试。"
                    progress = {
