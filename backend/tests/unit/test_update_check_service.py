...
from services.update_check_service import check_for_update, fetch_docker_hub_tags, get_current_version_metadata
...
def test_check_for_update_reports_newer_docker_image(monkeypatch):
    ...
    assert result["latest"]["image"] == "easyslide/easyslide:latest"


def test_fetch_docker_hub_tags_uses_easyslide_user_agent():
    tags = [_tag("latest", "sha256:latest")]

    with patch("services.update_check_service.requests.get", return_value=_mock_response(tags)) as request_get:
        fetch_docker_hub_tags()

    request_get.assert_called_once()
    assert request_get.call_args.kwargs["headers"]["User-Agent"] == "easyslide-updater"
