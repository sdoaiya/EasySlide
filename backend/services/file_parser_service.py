...
class FileParserService:
    ...
    def parse_file(self, file_path: str, filename: str) -> tuple[Optional[str], Optional[str], Optional[str], Optional[str], int]:
        ...
        try:
            ...
            # PDF uses built-in PaddleOCR-VL by default; layout extraction can opt into MinerU.
            if file_ext == 'pdf' and self.pdf_parser != 'mineru':
                logger.info(f"File {filename} is a PDF file, using PaddleOCR-VL...")
                return self._parse_pdf_file(file_path, filename)
