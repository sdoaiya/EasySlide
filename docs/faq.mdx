...
```bash
docker logs --tail 200 easyslide-backend
```
...
- Docker users who need PPTX support inside the container:
  ```bash
  docker exec -it easyslide-backend bash -c "apt-get update && apt-get install -y libreoffice-impress"
  ```
...
- If you're on a free API tier, rate limits may apply. Consider upgrading your OpenAI plan or reducing concurrent workers.
