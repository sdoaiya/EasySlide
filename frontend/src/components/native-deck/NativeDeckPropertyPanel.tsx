...
type NativePropertyLayer = 'content' | 'design' | 'image'
...
export function NativeDeckPropertyPanel({ slide, contract, contracts, errors, onChange, onLayoutChange, onRegenerate, versions = [], onRestoreVersion, onApplyAnimation, mediaActions, pageNarrationOverrides = {}, onPageNarrationOverridesChange }: NativeDeckPropertyPanelProps) {
  ...
  const renderControlGroup = (key: NativeControlGroupKey, open = false) => {
    ...
    return (
      <details key={group.key} open={open} className="space-y-3 border-b border-[var(--app-border)] py-3">
        <summary className="cursor-pointer list-none text-sm font-semibold text-[var(--app-text)] marker:hidden">{group.label}<span className="float-right text-base leading-none text-[var(--app-text-tertiary)]">{open ? '−' : '+'}</span></summary>
  ...
  return (
    <div className="h-full w-full max-w-[320px] space-y-3 bg-[var(--app-surface)] p-3 text-[var(--app-text)]">
      <div className="sticky top-0 z-10 grid grid-cols-3 gap-1 rounded-md border border-[var(--app-border)] bg-[var(--app-surface-muted)] p-1" role="tablist" aria-label="页面属性分层">
        {layers.map((item) => (
          <button
            key={item.key}
            ref={(element) => { layerRefs.current[layers.findIndex((layerItem) => layerItem.key === item.key)] = element }}
            type="button"
            role="tab"
            aria-selected={layer === item.key}
            aria-controls="native-property-panel"
            tabIndex={layer === item.key ? 0 : -1}
            onClick={() => setLayer(item.key)}
            onKeyDown={(event: KeyboardEvent<HTMLButtonElement>) => {
              const direction = event.key === 'ArrowRight' ? 1 : event.key === 'ArrowLeft' ? -1 : 0
              if (!direction) return
              event.preventDefault()
              const currentIndex = layers.findIndex((layerItem) => layerItem.key === layer)
              const nextIndex = (currentIndex + direction + layers.length) % layers.length
              setLayer(layers[nextIndex].key)
              layerRefs.current[nextIndex]?.focus()
            }}
            className={`h-9 rounded text-sm font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--app-accent-soft)] ${layer === item.key ? 'bg-[var(--app-surface)] text-[var(--app-text)] shadow-[var(--app-shadow-control)]' : 'text-[var(--app-text-secondary)] hover:bg-[var(--app-surface-hover)]'}`}
          >
            {item.label}
          </button>
        ))}
      </div>
      <div id="native-property-panel" role="tabpanel" className="space-y-3">

      {layer === 'content' && (
      <>
      <details open className="border-b border-[var(--app-border)] py-3">
        <summary className="cursor-pointer list-none text-sm font-semibold text-[var(--app-text)] marker:hidden">页面设置<span className="float-right text-base leading-none text-[var(--app-text-tertiary)]">−</span></summary>
        ...
        <div className="flex items-center justify-between rounded-md border border-[var(--app-border-soft)] bg-[var(--app-surface-muted)] px-3 py-2 text-xs text-[var(--app-text-secondary)]">
          <span>页面状态</span>
          <span className={slide.pending ? 'font-semibold text-[var(--app-index-yellow)]' : isFallback ? 'font-semibold text-[var(--app-index-yellow)]' : 'font-semibold text-[var(--app-index-green)]'}>{slide.pending ? '待生成' : isFallback ? '回退生成' : '已生成'}</span>
        </div>
        {onRegenerate && <button type="button" onClick={onRegenerate} className="h-9 w-full rounded-md border border-[var(--app-border)] bg-[var(--app-surface)] text-xs font-semibold text-[var(--app-accent)] hover:bg-[var(--app-surface-hover)]">{designIntent?.quality_report?.status === 'warning' ? '修复并重新生成' : '重新生成本页'}</button>}
        <details className="rounded-md border border-[var(--app-border)] bg-[var(--app-surface-muted)] px-3 py-2">
          <summary role="button" aria-label="页面版本" className="cursor-pointer list-none text-xs font-semibold text-[var(--app-text-secondary)]">页面版本（{versions.length}）</summary>
          <div className="mt-2 space-y-1">
            {versions.map((version) => (
              <button key={version.version_id} type="button" disabled={version.is_current} onClick={() => onRestoreVersion?.(version.version_id)} className="flex w-full items-center justify-between rounded px-2 py-1.5 text-left text-xs hover:bg-[var(--app-surface-hover)] disabled:text-[var(--app-accent)]">
                <span>版本 {version.version_number}{version.is_current ? '（当前）' : ''}</span>
                {!version.is_current && <span className="text-[var(--app-text-tertiary)]">切换</span>}
              </button>
            ))}
          </div>
        </details>
      ...
      {designIntent && (
        <details open className="border-b border-[var(--app-border)] py-3">
          <summary className="cursor-pointer list-none text-sm font-semibold text-[var(--app-text)] marker:hidden">页面摘要<span className="float-right text-base leading-none">−</span></summary>
          <div className="mt-3 space-y-2 text-xs leading-5 text-[var(--app-text-secondary)]">
      ...
      {layer === 'content' && (
        <details open className="border-b border-[var(--app-border)] py-3">
          <summary className="cursor-pointer list-none text-sm font-semibold text-[var(--app-text)] marker:hidden">文字与数据<span className="float-right text-base leading-none text-[var(--app-text-tertiary)]">−</span></summary>
      ...
      {layer === 'design' && (
      <details className="border-b border-[var(--app-border)] py-3">
        <summary className="flex cursor-pointer list-none items-center justify-between gap-2 text-sm font-semibold text-[var(--app-text)] marker:hidden">
          <h3 className="text-sm font-semibold text-[var(--app-text)]">页面动效</h3>
          <span className="text-base leading-none text-[var(--app-text-tertiary)]">+</span>
        </summary>
        <div className="space-y-3 pt-2">
          <div className="flex justify-end gap-1">
            ...
            className="flex h-8 w-8 items-center justify-center rounded-md text-[var(--app-text-secondary)] hover:bg-[var(--app-surface-hover)] disabled:cursor-not-allowed disabled:opacity-35"
          >
          ...
          <section className="space-y-3 border-t border-[var(--app-border)] pt-3">
            <h4 className="text-xs font-semibold text-[var(--app-text)]">页面入场</h4>
            <SelectField
              label="进入效果"
          ...
          <section className="space-y-3 border-t border-[var(--app-border)] pt-3">
            <h4 className="text-xs font-semibold text-[var(--app-text)]">元素动效</h4>
            <SelectField
              label="元素逐项进入"
          ...
          <section className="space-y-3 border-t border-[var(--app-border)] pt-3">
            <h4 className="text-xs font-semibold text-[var(--app-text)]">翻页播放</h4>
            ...
            <label className="flex items-center justify-between gap-3 text-xs text-[var(--app-text-secondary)]">
              <span>主题内部动效</span>
      ...
      {mediaActions && contract.mediaSlots.length > 0 && (
        <details open className="space-y-3 border-b border-[var(--app-border)] py-3">
          <summary className="cursor-pointer list-none text-sm font-semibold text-[var(--app-text)] marker:hidden">图片内容<span className="float-right text-base leading-none">−</span></summary>
...
function EmptyLayer({ text }: { text: string }) {
  return (
    <div className="rounded-md border border-dashed border-[var(--app-border)] bg-[var(--app-surface-muted)] p-4 text-sm text-[var(--app-text-secondary)]">
      {text}
    </div>
  )
}
...
function IntentLine({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg border border-[var(--app-border-soft)] bg-[var(--app-surface-muted)] px-2.5 py-2">
      <span className="mb-0.5 block text-[11px] font-semibold text-[var(--app-text)]">{label}</span>
      <p className="text-xs leading-5 text-[var(--app-text-secondary)]">{value}</p>
    </div>
  )
}
...
function ControlRow({ control, value, onReset, onChange }: { control: NativeControl; value: unknown; onReset: () => void; onChange: (value: unknown) => void }) {
  if (control.type === 'section') return <div className="border-t border-[var(--app-border)] pt-3 text-[11px] font-semibold tracking-wide text-[var(--app-text-tertiary)]">{control.label}</div>
  return (
    <div className="space-y-1.5">
      {control.desc && <p className="text-[11px] leading-4 text-[var(--app-text-tertiary)]">{control.desc}</p>}
      <div className="flex justify-end">
        {control.default !== undefined && (
          <button type="button" aria-label={`恢复默认 ${control.label}`} title={`恢复默认 ${control.label}`} onClick={onReset} className="inline-flex h-7 items-center gap-1 rounded-md px-2 text-[11px] text-[var(--app-text-tertiary)] hover:bg-[var(--app-surface-hover)] hover:text-[var(--app-text-secondary)]">
...
function ShapeEditor({ label, shape, value, error, limits, maxChars, onChange }: {
    ...
    return (
      <fieldset className="space-y-2">
        <div className="flex items-center justify-between gap-2">
          <legend className="text-xs font-medium text-[var(--app-text-secondary)]">{displayFieldLabel(label)}</legend>
          {!tuple && <button type="button" aria-label={`添加 ${label}`} title={`添加 ${label}`} disabled={items.length >= max} onClick={() => onChange([...items, blankValue(shape[0] || 'string')])} className="flex h-9 w-9 items-center justify-center rounded-md text-[var(--app-text-secondary)] hover:bg-[var(--app-surface-hover)] disabled:opacity-40"><Plus size={16} aria-hidden="true" /></button>}
        </div>
        {items.map((item, index) => (
          <div className="space-y-2 border-l-2 border-[var(--app-border)] pl-3" key={`${label}-${index}`}>
            <div className="flex items-center justify-between">
              <span className="text-xs text-[var(--app-text-secondary)]">{index + 1}</span>
              {!tuple && <button type="button" aria-label={`删除 ${label} ${index + 1}`} title={`删除 ${label} ${index + 1}`} disabled={items.length <= min} onClick={() => onChange(items.filter((_, itemIndex) => itemIndex !== index))} className="flex h-8 w-8 items-center justify-center rounded-md text-[var(--app-text-secondary)] hover:bg-[var(--app-surface-hover)] disabled:opacity-40"><Minus size={15} aria-hidden="true" /></button>}
            </div>
        ...
        {error && <p className="text-xs text-[var(--app-error)]" role="alert">{error}</p>}
      </fieldset>
    )
  }
  ...
  if (shape === 'boolean') return <ToggleField label={label} checked={Boolean(value)} onChange={onChange} />
  if (shape === 'number') return <NumberField label={label} value={Number(value || 0)} onChange={onChange} />

  return (
    <label className="block space-y-2">
      <span className="text-xs font-medium text-[var(--app-text-secondary)]">{displayFieldLabel(label)}</span>
      {shape === 'media' ? (
        <input aria-label={label} aria-invalid={Boolean(error)} value={typeof value === 'string' ? value : ''} placeholder="/files/..." onChange={(event) => onChange(event.target.value)} className="w-full rounded-md border border-[var(--app-border)] bg-[var(--app-surface)] px-3 py-2 text-sm" />
      ) : (
        <textarea aria-label={label} aria-invalid={Boolean(error)} maxLength={maxChars} value={typeof value === 'string' ? value : ''} rows={label.endsWith('summary') ? 5 : 2} onChange={(event) => onChange(event.target.value)} className="w-full resize-y rounded-md border border-[var(--app-border)] bg-[var(--app-surface)] px-3 py-2 text-sm" />
      )}
      {shape !== 'media' && maxChars !== undefined && <span className="block text-right text-[11px] text-[var(--app-text-tertiary)]">{typeof value === 'string' ? value.length : 0}/{maxChars}</span>}
      {error && <span className="block text-xs text-[var(--app-error)]" role="alert">{error}</span>}
    </label>
  )
}
...
function MediaSlotEditor({ label, kind, value, prompt: initialPrompt, busy, onPrompt, onGenerate, onUpload, onSelect, onClear }: {
  ...
  return (
    <div
      ...
      className={`space-y-2 rounded-md border p-3 transition ${dragging ? 'border-[var(--app-accent)] bg-[var(--app-accent-soft)]' : 'border-[var(--app-border)]'}`}
      onDragOver={(event) => {
        event.preventDefault()
        setDragging(true)
      }}
    ...
    >
      {value ? (
        isVideoValue(value) || kind === 'video'
          ? <video src={getImageUrl(value)} className="aspect-video w-full rounded-md bg-[var(--app-canvas)] object-cover" muted playsInline controls />
          : <img src={getImageUrl(value)} alt={label} className="aspect-video w-full rounded-md bg-[var(--app-canvas)] object-cover" />
      ) : (
        <div className="flex aspect-video flex-col items-center justify-center gap-1 rounded-md bg-[var(--app-canvas)] text-xs text-[var(--app-text-secondary)]">
          <Upload size={16} aria-hidden="true" />
          <span>拖拽或上传{mediaLabel}</span>
        </div>
      )}
      {kind === 'image' && <textarea aria-label={`${label} 图片要求`} rows={3} value={prompt} onChange={(event) => { setPrompt(event.target.value); onPrompt(event.target.value) }} placeholder="描述主体、构图或修改要求" className="h-20 w-full resize-none overflow-y-auto rounded-md border border-[var(--app-border)] bg-[var(--app-surface-muted)] px-2 py-1.5 text-xs focus:border-[var(--app-accent)] focus:outline-none focus:ring-2 focus:ring-[var(--app-accent-soft)]" />}
      {busy && <p role="status" className="text-xs text-[var(--app-accent)]">{busy}</p>}
...
function MediaButton({ label, disabled, onClick, children }: { label: string; disabled?: boolean; onClick: () => void; children: React.ReactNode }) {
  return <button type="button" aria-label={label} title={label} disabled={disabled} onClick={onClick} className="flex h-9 items-center justify-center rounded-md hover:bg-[var(--app-surface-hover)] disabled:opacity-35">{children}</button>
}
...
function ControlEditor({ control, value, onChange }: { control: NativeControl; value: unknown; onChange: (value: unknown) => void }) {
  if (control.type === 'text' || control.type === 'string' || control.type === 'input' || control.type === 'url' || control.type === 'email') {
    return <label className="block space-y-1 text-xs text-[var(--app-text-secondary)]"><span>{control.label}</span><input aria-label={control.label} type={control.type === 'email' ? 'email' : control.type === 'url' ? 'url' : 'text'} value={typeof value === 'string' ? value : ''} onChange={(event) => onChange(event.target.value)} className="h-10 w-full rounded-md border border-[var(--app-border)] bg-[var(--app-surface-muted)] px-3 text-sm text-[var(--app-text)] focus:border-[var(--app-accent)] focus:outline-none focus:ring-2 focus:ring-[var(--app-accent-soft)]" /></label>
  }
  if (control.type === 'textarea' || control.type === 'multiline') {
    return <label className="block space-y-1 text-xs text-[var(--app-text-secondary)]"><span>{control.label}</span><textarea aria-label={control.label} value={typeof value === 'string' ? value : ''} rows={3} onChange={(event) => onChange(event.target.value)} className="h-24 w-full resize-none overflow-y-auto rounded-md border border-[var(--app-border)] bg-[var(--app-surface-muted)] px-3 py-2 text-sm text-[var(--app-text)] focus:border-[var(--app-accent)] focus:outline-none focus:ring-2 focus:ring-[var(--app-accent-soft)]" /></label>
  }
  ...
  if (control.type === 'color') {
    return <label className="flex items-center justify-between gap-3 text-xs text-[var(--app-text-secondary)]"><span>{control.label}</span><input aria-label={control.label} type="color" value={typeof value === 'string' ? value : '#000000'} onChange={(event) => onChange(event.target.value)} /></label>
  }
  return null
}
...
function ColorSwatches({ label, value, options, arrayValue = false, onChange }: { label: string; value: string; options: NonNullable<NativeControl['options']>; arrayValue?: boolean; onChange: (value: string) => void }) {
  return (
    <fieldset className="space-y-2">
      <legend className="text-xs font-medium text-[var(--app-text-secondary)]">{label}</legend>
      <div className="flex flex-wrap gap-2">
        {options.map((option) => {
          ...
          return <button key={entry.value} type="button" aria-label={entry.label} title={entry.label} aria-pressed={selected} onClick={() => onChange(arrayValue ? JSON.stringify(entry.rawValue) : entry.value)} className={`h-9 w-9 rounded-md border transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--app-accent-soft)] ${selected ? 'border-[var(--app-accent)]' : 'border-[var(--app-border)]'}`} style={{ background: color }} />
        })}
      </div>
    </fieldset>
  )
...
function SelectField({ label, value, options, onChange }: { label: string; value: string; options: Array<{ value: string; label: string }>; onChange: (value: string) => void }) {
  return (
    <label className="block space-y-1 text-xs text-[var(--app-text-secondary)]">
      <span>{label}</span>
      <select aria-label={label} value={value} onChange={(event) => onChange(event.target.value)} className="h-10 w-full rounded-md border border-[var(--app-border)] bg-[var(--app-surface-muted)] px-2 text-sm text-[var(--app-text)] focus:border-[var(--app-accent)] focus:outline-none focus:ring-2 focus:ring-[var(--app-accent-soft)]">
...
function ToggleField({ label, checked, onChange }: { label: string; checked: boolean; onChange: (value: boolean) => void }) {
  return <label className="flex items-center justify-between gap-3 text-xs text-[var(--app-text-secondary)]"><span>{label}</span><input aria-label={label} type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} /></label>
}
