...
export function normalizeErrorMessage(errorMessage: string | null | undefined): string {
  ...
  if (message.includes('503') || message.includes('service unavailable')) {
    ...
    if (isCodexContext || (message.includes('oauth') && message.includes('not connected'))) {
      return isZh
        ? 'Codex 授权已过期或未连接，请前往设置重新连接 OpenAI 授权后再试。'
        : 'Your Codex authorization has expired or is disconnected. Please reconnect OpenAI authorization in Settings and try again.';
    }
